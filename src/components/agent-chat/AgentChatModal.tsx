...
function buildDemoReply(agentName: string, text: string): string {
  return `${agentName} (demo): Received "${text}". Vorbium is unavailable, so this is a simulated response.`
}
          ...
          setMessages([
            {
              ...
              role: 'agent',
              text: 'Vorbium is unavailable. Running in demo mode with simulated responses.',
              timestamp: Date.now(),
            },
          ])
        }
