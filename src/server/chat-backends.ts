import { streamChat } from './vorbium-api'
...
async function* streamVorbiumChat(
  messages: Array<ChatMessage>,
  options: UnifiedChatOptions,
): AsyncGenerator<string, void, void> {
  if (!options.sessionId) {
    throw new Error('Vorbium enhanced chat requires sessionId')
  }
  ...
  if (!lastUserMessage) {
    throw new Error('Vorbium enhanced chat requires a user message')
  }
...
export async function sendChatUnified(
  ...
  if (backend === 'vorbium-enhanced') {
    let text = ''
    for await (const delta of streamVorbiumChat(messages, options)) {
      text += delta
    }
    return text
  }
...
export async function streamChatUnified(
  ...
  if (backend === 'vorbium-enhanced') {
    return streamVorbiumChat(messages, options)
  }
