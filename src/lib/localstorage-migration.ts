/**
 * Migração one-shot de localStorage keys hermes-* → vorbium-*.
 * Roda uma vez por carga; se a chave nova já existe, não sobrescreve.
 * Remove as chaves antigas depois.
 */

const MIGRATIONS: Record<string, string> = {
  'hermes-theme': 'vorbium-theme',
  'hermes-workspace-v1': 'vorbium-workspace-v1',
  'hermes-onboarding-complete': 'vorbium-onboarding-complete',
  'hermes-sidebar-last-route': 'vorbium-sidebar-last-route',
  'hermes-workspace-locale': 'vorbium-workspace-locale',
  'hermes-auth': 'vorbium-auth',
}
...
export function migrateHermesLocalStorage(): void {
  ...
  if (migrated > 0 && typeof console !== 'undefined') {
    ...
    console.log(`[Vorbium] Migrated ${migrated} localStorage key(s) from hermes-* to vorbium-*`)
  }
}
