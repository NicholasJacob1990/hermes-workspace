# Vorbium Engine Workspace OpenAI-Compat Architecture Spec

> **For Vorbium Engine:** Use `writing-plans` if this turns into an implementation plan. This doc locks the product and backend compatibility direction.

**Goal:** Make Vorbium Engine Workspace work out of the box against vanilla `hermes-agent` and any OpenAI-compatible backend, while unlocking richer workspace features automatically when Vorbium Engine-specific APIs are available.
...
Vorbium Engine Workspace currently depends on a forked `hermes-agent` gateway for extended functionality:
...
> **Vorbium Engine Workspace must work standalone against any OpenAI-compatible backend.**
>
> Vorbium Engine-specific workspace features may enhance the experience when the full Vorbium Engine API is available, but the product must remain usable without those endpoints.
...
In this mode, advanced features degrade gracefully when Vorbium Engine-specific APIs are absent.

### Step 2 — Upstream the richer API later

Submit the custom Vorbium Engine endpoints into upstream `hermes-agent`, targeting `gateway/platforms/api_server.py`.
...
- `/api/memory`
- `/api/config`
- Vorbium Engine-specific metadata endpoints

### Mode B — Enhanced Vorbium Engine Mode

When Vorbium Engine-specific endpoints are present, unlock:
...
If a user points Vorbium Engine Workspace at a valid OpenAI-compatible backend, they should be able to send a message and receive a streamed response without caring whether the backend is Vorbium Engine, OpenAI, OpenRouter, Ollama, vLLM, or something else.
...
1. Detect whether Vorbium Engine session APIs exist.
2. If yes, use the enhanced Vorbium Engine session flow.
...
- missing Vorbium Engine sessions API must no longer cause the product to hang or hard-fail for basic chat
...
#### Vorbium Engine enhancement capabilities
...
When Vorbium Engine-specific APIs are missing, the UI must not show broken loaders, dead tabs, or cryptic errors.
...
- verify chat works
- then advertise extra Vorbium Engine-native features if supported

Onboarding copy should communicate:

- “Works with any OpenAI-compatible backend”
- “Enhanced features unlock automatically with Vorbium Engine gateway APIs”
...
- the richer Vorbium Engine API is optional for advanced workspace features
...
- **Enhanced** — Vorbium Engine workspace APIs detected
...
- “Memory browser requires Vorbium Engine memory API.”
...
When no Vorbium Engine sessions API exists, the app still needs a sane chat UX.
...
### 8.2 Enhanced Vorbium Engine path

Enhanced path remains Vorbium Engine-native where available, because it provides:
...
- let Vorbium Engine Workspace treat stock Vorbium Engine as the best backend, without requiring it
...
- guaranteed session persistence on non-Vorbium Engine backends
- memory/skills/config support outside Vorbium Engine
- building a backend abstraction for every vendor-specific extension

The goal is simpler:

- portable chat first
- enhanced Vorbium Engine features second
- no fork requirement
...
- A user can launch Vorbium Engine Workspace against a stock OpenAI-compatible backend and successfully chat without patching backend code.
- A user can launch Vorbium Engine Workspace against vanilla `hermes-agent` and get a working core experience.
- Advanced features do not hard-fail the app when Vorbium Engine-specific APIs are absent.
- The UI clearly communicates portable mode vs enhanced Vorbium Engine mode.

### Technical acceptance

- Chat send path no longer hard-depends on `/api/sessions`.
- Capability probing includes `/v1/chat/completions` readiness, not just Vorbium Engine-specific APIs.
...
- Enhanced Vorbium Engine API support is documented as progressive enhancement.
...
1. Separate **core chat client** from **Vorbium Engine enhanced client**.
...
7. After Step 1 is stable, prepare the upstream PR for Vorbium Engine-native endpoints.
...
> Vorbium Engine Workspace is a standalone frontend for OpenAI-compatible chat backends.
>
> Vorbium Engine-native APIs are an enhancement layer, not a requirement.
>
> Step 1 is portable compatibility now.
>
> Step 2 is upstreaming the enhanced Vorbium Engine APIs so no fork is needed ever again.
