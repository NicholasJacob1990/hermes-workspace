...
const config = defineConfig(({ mode, command }) => {
  ...
  const startVorbiumEngine = async () => {
    ...
    if (hermesBin) {
      ...
      console.log(`[vorbium-engine] Starting ${hermesBin} gateway run`)
    } else if (agentDir) {
      launchCmd = resolveVorbiumPython(agentDir)
      ...
      launchCwd = agentDir
      console.log(
        `[vorbium-engine] Starting from ${agentDir} using ${launchCmd} (${useGatewayRun ? 'gateway.run' : 'uvicorn'})`,
      )
    } else {
      console.warn(
        '[vorbium-engine] Could not find hermes-agent directory.\n' +
          '  Set HERMES_AGENT_PATH or VORBIUM_ENGINE_PATH in .env or clone hermes-agent as a sibling:\n' +
